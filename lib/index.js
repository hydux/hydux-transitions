...
exports.Units = {
    ...
    cubicBezier: function (ax, ay, bx, by) {
...
var Actions = /** @class */ (function () {
    function Actions() {
        /** @internal */
        this.$startFrame = function (frame) { return function (state) {
            var nextState = tslib_1.__assign({}, state, { style: frameToStyle(frame), className: frame.className || '' });
            return [nextState, frame.onStart ? hydux_1.Cmd.ofFn(frame.onStart, frame) : hydux_1.Cmd.none];
        }; };
        // @internal
        this.$endFrame = function (frame) { return function (state) {
            return [state, frame.onEnd ? hydux_1.Cmd.ofFn(frame.onEnd, frame) : []];
        }; };
        // @internal
        this.$end = function () { return function (state) { return (tslib_1.__assign({}, state, { animState: AnimState.end })); }; };
        this.start = function (onEnd) { return function (state) {
            return [
                exports.init.apply(null, state._initArgs),
                hydux_1.Cmd.ofSub(function (actions) {
                    requestAnimationFrame(function () { return runFrames(state.frames, state, actions, onEnd); });
                }),
            ];
        }; };
        this.run = function (frames) { return function (state) {
